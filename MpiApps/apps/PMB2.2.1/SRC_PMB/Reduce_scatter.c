...
/* Prototype Reduce_scatter (this file)  */
...
/******* Implementation Reduce_scatter *******/

void Reduce_scatter(struct comm_info* c_info,int size,int n_sample,
                    MODES RUN_MODE,double* time)
/******************************************************************************
...
defect=0.;
  ...
  for (i=0;i<c_info->num_procs ;i++)
    {
     get_rank_portion(i, c_info->num_procs, size, s_size, 
                      &pos1, &pos2);
